...
export default function EditItem() {
      ...
      if (data.status === 'success') {
        ...
        if (foundItem) {
          setItem({
            ...
            mrp: foundItem.mrp ? foundItem.mrp.toString() : '',
            quantity_in_stock: foundItem.quantity_in_stock.toString(),
            tags: foundItem.tags || ''
          });
        } else {
  ...
  const updateItem = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backend}/item/update/${id}`, {
        method: 'POST',
        headers: {
  ...
  if (!item) {
    return (
      <div className="screen-content">
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>❌</div>
          <p style={{ color: 'var(--text-secondary)' }}>Item not found</p>
        </div>
      </div>
    );
  }
    ...
    <div className="screen-content" style={{ paddingBottom: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12, marginTop: 24 }}>
          ...
          <button
            className={`btn btn-primary btn-large ${saving ? 'opacity-50' : ''}`}
            onClick={updateItem}
            disabled={saving || !item.title.trim() || !item.price}
            style={{ 
              fontSize: 16,
              padding: '16px 0',
              opacity: saving || !item.title.trim() || !item.price ? 0.5 : 1
            }}
          >
            {saving ? 'Updating Item...' : 'Update Item'}
          </button>
        </div>
      </div>
    </div>
